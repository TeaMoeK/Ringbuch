...
namespace Ringbuch
{
  public partial class ErgebnisBearbeiten : Form, ErgebnisBearbeitenInterface
  {
    ...
    {
      InvokeSchiessartDeleteRequested(Convert.ToInt32(comboArt.SelectedValue));
      schiessartenLoad();
    }
    public event EventHandler<IDEventArgs> SchiessartDeleteRequested;
    private void InvokeSchiessartDeleteRequested(int schiessArtID)
    {
      EventHandler<IDEventArgs> handler = SchiessartDeleteRequested;
      if (handler != null)
      {
        SchiessartDeleteRequested(this, new IDEventArgs(schiessArtID, "schiessart"));
      }
    }
    /// <summary>
    /// Schiessart Neu
